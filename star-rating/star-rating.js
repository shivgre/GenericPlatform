            ...
            self.$rating.on("click", function (e) {
			   e.preventDefault();
                if (!self.inactive) {
					...
					var tuid11 = self.$element.attr("data-id");
					var dataFrom = self.$element.attr("data-from");
					var action11 = "project_rating";
					if(dataFrom == "user"){
						action11  = "user_rating";
					}
					var rating11 = self.$element.val();
					$.post(
						'star-rating/star-rating-ajax.php',
						{tuid:tuid11, rating:rating11, action:action11},
						function(data){
							if(data){
								 //alert('test');
							}else{
