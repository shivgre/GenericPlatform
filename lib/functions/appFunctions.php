...
function sortImg($column,$sortID,$count,$tableSORT){
	...
	if ($sortID != "default") {
		if ($column == $sortID) {
			if ($tableSORT == "DESC") {
				$sortImg = "<img style='padding-left:5px;' src=\"resources/images/bullet_arrow_down.png\">";
			} else {
				$sortImg = "<img style='padding-left:5px;' src=\"resources/images/bullet_arrow_up.png\">";
			}
		}
	} else {
		if ($count == 0) {
...
function getNewSortOrder($tableSORT){
	$newSortOrder="";
	if (isset ( $tableSORT )) {
		if ($tableSORT == "ASC") { // toggleSortMe(pagenum,tablename,sortid,sortorder,where)
			$newSortOrder="DESC";
		} else {
			$newSortOrder="ASC";
		}
...
function getSingleSimulationTabData($config,$colidx,$idxData,$tableAlias){
	...
	while ($length>$count ) {
		...
		if ($relatonalData != '' && $rows[$count][0] != "UserIDs") { //// Generation for general IDS relational
