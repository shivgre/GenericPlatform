...
//$_SESSION['update_table']['parent_key'] = 'id';
    ...
    $urow = get_single_record($row1['database_table_name'], $row1['parent_key'], $_SESSION['search_id']);
...
///rs use
    ...
    if (( ( $row1['list_views'] == 'NULL' || $row1['list_views'] == '' ) || ( isset($_GET['id'])) || $_GET['edit'] == 'true') && $row1['table_type'] != 'content') {
        ...
        if ($row1['dd_editable'] == 1) {
            echo "<button class='btn btn-default pull-right edit-btn' id='$row1[dict_id]'>" . EDIT . "</button>";
        }

        if (isset($_SESSION['return_url']) && $_GET['checkFlag'] == 'true') {
            echo "<form action='?action=update&checkFlag=true&tabNum=$row1[tab_num]' method='post' id='user_profile_form' enctype='multipart/form-data' class='$style'><br>";
        } else {
            echo "<form action='?action=update&tabNum=$row1[tab_num]' method='post' id='user_profile_form' enctype='multipart/form-data' class='$style'><br>";
        }
...
function formating_Update($row, $urow = 'false') {
...
function image_upload($row, $urow = 'false') {
    ...
    echo "<div class='left-content'><div class='img-holder' >
        
  <span> <img src='" . BASE_URL . "users_uploads/" . $img_show . "' border='0' width='150' class='img-thumbnail img-responsive user_thumb' /> </span>";

    if (!empty($img)) {
        echo " <button class='btn btn-primary update-btn remove-img-btn' id='$row[generic_field_name]' type='button'>REMOVE</button>";
    } else {

        echo "<input type='hidden' role='uploadcare-uploader' name='image' class='file2' id='file2' data-locale='en' data-tabs='file url facebook gdrive instagram' data-images-only='false' data-path-value='false' data-preview-step='false' data-multiple='false'  value='' data-crop='650x430 minimum'/>
                <br />
             
                <input type='hidden' name='field_name_unique' class='field_name' value='$row[generic_field_name]' />
                <div style='margin:5% 0%'>
                    <input type='button' class='submit btn btn-primary pull-left login-btn' name='profile_image_submit'  value='SAVE'>
                    <input type='button' onclick='location.href' = 'profile.php'' class='submit btn btn-primary  pull-right' name='login' value='CANCEL'/>
                </div>
...
function list_display($qry, $tab_num = 'false') {
            ...
            if (strpos($list_select, '.php')) {
                ...
                if (isset($list_select_arr[0]) && !empty($list_select_arr[0])) {
                    ...
                    if (count($list_select_arr[0]) == 2) {
                        ...
                        /// add button url
                        $_SESSION['add_url_list'] = BASE_URL . $navList['item_target'] . "?display=" . $navList['target_display_page'] . "&tab=" . $list_select_arr[0][0] . "&tabNum=" . $list_select_arr[0][1] . "&layout=" . $navList['page_layout_style'] . "&style=" . $navList['item_style'] . "&addFlag=true";
                    } else {
                        $target_url = BASE_URL . "system/profile.php?display=" . $list_select_arr[0][2] . "&tab=" . $list_select_arr[0][0] . "&tabNum=" . $list_select_arr[0][1] . "&search_id=" . $listRecord[$parent_key] . "&checkFlag=true";
                ...
                if (isset($ret_array['single_delete_array']) && !empty($ret_array['single_delete_array'])) {
                    ...
                    if ($ret_array['single_delete_array']['loc'] == 'right') {
                        ...
                        echo "<span class='glyphicon glyphicon-edit list-copy pull-right $sing_del_style' title-'Delete' id='$checkbox_id'></span>";
                    } else {
                        echo "<span class='glyphicon glyphicon-edit list-copy $sing_del_style' title-'Delete' id='$checkbox_id'></span>";
                    }
                }
...
//print_r($list_extra_options);die;
    ...
    return array("del_array" => $del_array, "copy_array" => $copy_array, "add_array" => $add_array, "single_delete_array" => $single_delete_array, "single_copy_array" => $single_copy_array, "checklist_array" => $checklist_array, "popupmenu" => $popupmenu, "popup_delete" => $popup_delete_array, "popup_copy" => $popup_copy_array);
}
